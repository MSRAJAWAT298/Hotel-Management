...
//$join="country.country_name,state.state_name,city.city_name";
//$table_name="employee";
//$con= "AND emp_id=$id";
/*$join_condition="
               left join country on country.country_id = employees.country 
               left join state on state.state_id = employees.state 
               left join city on city.city_id = employees.city ";*/

      function selectjoin($table_name,$con="",$join="*",$join_condition="")
    {
        $sql = "SELECT $join FROM $table_name $join_condition WHERE 1 $con";
        $stmt = $this->conn->query($sql); 
